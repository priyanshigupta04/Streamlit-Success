...
// const [docFormData, setDocFormData] = useState({});
  ...
  const getDocRowId = (req) => req?._id || req?.id || '';

  const filteredDocRequests = docRequests.filter((req) => {
    if (docFilter === 'all') return true;
    return req.overallStatus === docFilter;
  });

  const allFilteredSelected =
    filteredDocRequests.length > 0 &&
    filteredDocRequests.every((req) => selectedDocIds.includes(getDocRowId(req)));
  ...
  const toggleSelectAllFilteredDocs = () => {
    if (!filteredDocRequests.length) return;
    const filteredIds = filteredDocRequests
      .map((req) => getDocRowId(req))
      .filter(Boolean);
  ...
  const handleDeleteSelectedDocs = async () => {
    if (!selectedDocIds.length || deletingDocs) return;

    const validDocIds = selectedDocIds.filter((id) => /^[a-f\d]{24}$/i.test(String(id || '')));
    if (!validDocIds.length) {
      alert('No valid document selected for deletion. Please refresh and try again.');
      return;
    }
    ...
    try {
      ...
      const results = await Promise.allSettled(
        validDocIds.map((id) => axios.delete(`/api/documents/${id}`))
      );
      ...
      if (failureCount > 0) {
        ...
        const failureMessage = firstFailure?.reason?.response?.data?.message || firstFailure?.reason?.message || 'Some requests could not be deleted';
        alert(`Deleted ${successCount} request(s). ${failureCount} request(s) could not be deleted. ${failureMessage}`);
      } else {
        alert(`Deleted ${successCount} request(s) successfully.`);
      }

      setSelectedDocIds([]);
      await fetchDocuments();
    } catch (err) {
      alert(err.response?.data?.message || 'Failed to delete selected document requests');
    } finally {
        ...
        <div className="space-y-8 fade-in">
          ...
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex items-center gap-3">
                <select
                  ...
                  className="text-[11px] font-black uppercase px-3 py-2 rounded-xl border border-slate-200 bg-white outline-none"
                >
                ...
                <label className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-600 px-3 py-2 rounded-xl border border-slate-200 bg-slate-50">
                  <input
                    type="checkbox"
                    checked={allFilteredSelected}
                    onChange={toggleSelectAllFilteredDocs}
                    disabled={!filteredDocRequests.length}
                ...
                <button
                  ...
                  disabled={!selectedDocIds.length || deletingDocs}
                  className="px-3 py-2 rounded-xl text-[10px] font-black uppercase bg-rose-600 text-white disabled:bg-slate-300 disabled:text-slate-500 transition-all flex items-center gap-2"
                  title="Delete selected document requests"
                >
                  <Trash2 size={14} />
                  {deletingDocs ? 'Deleting...' : `Delete (${selectedDocIds.length})`}
                </button>
            ...
            {docRequests.length === 0 ? (
              ...
              <div className="text-center py-12">
                <FileText size={40} className="mx-auto text-slate-300 mb-4"/>
                <p className="text-slate-500 font-bold">No document requests found for selected filter</p>
              </div>
            ) : (
              <div className="space-y-4">
                {filteredDocRequests.map(req => {
                  ...
                  const rowId = getDocRowId(req);
                  
                  return (
                    <div key={rowId} className="flex items-center justify-between p-6 bg-slate-50 rounded-[1.8rem] border border-slate-100 hover:border-black transition-all">
                      <div className="flex items-center gap-4 flex-1">
                        <input
                          type="checkbox"
                          checked={selectedDocIds.includes(rowId)}
                          onChange={() => toggleDocSelection(rowId)}
                          className="w-4 h-4 accent-black"
                          aria-label={`Select ${displayType}`}
                        />
  ...
  {showDeleteDocsConfirm && (
    <div className="fixed inset-0 z-[112] flex items-center justify-center p-6">
      <div
        ...
        onClick={() => !deletingDocs && setShowDeleteDocsConfirm(false)}
      ></div>
      <div className="relative z-10 w-full max-w-md bg-white rounded-[2rem] border border-slate-200 shadow-2xl p-8">
        <h3 className="text-xl font-black italic uppercase tracking-tighter">Confirm Delete</h3>
        <p className="text-sm text-slate-600 mt-3 font-semibold">
          Delete {selectedDocIds.length} selected document request(s)? This action cannot be undone.
        </p>
        ...
        <div className="mt-7 flex items-center justify-end gap-3">
          <button
            ...
            className="px-4 py-3 rounded-xl text-[11px] font-black uppercase bg-slate-100 text-slate-700 hover:bg-slate-200 transition-all disabled:opacity-60"
          >
            Cancel
          </button>
          <button
            ...
            disabled={deletingDocs || !selectedDocIds.length}
            className="px-4 py-3 rounded-xl text-[11px] font-black uppercase bg-rose-600 text-white hover:bg-rose-700 transition-all disabled:opacity-60"
          >
